...
//            }
//            con.Close();   TableEntity.Fields.Id, OperMath.Equal, 1
            var test = new TableEntity("test");
            var results = test
                .Select()
                .Where(new[] {
                    new FilterWhere(TableEntity.Fields.Id, OperMath.Equal, 1),
                    new FilterWhere(TableEntity.Fields.Id, OperMath.Equal, 1) })
                .GetData();
...
//            }
            ...
            test.Data = "saved";
            test.Save();
            results = test.Select().Where(TableEntity.Fields.Id, OperMath.Equal, 55).GetData();
            foreach (var result in results)
            {
                Console.WriteLine(result.Id + " " + result.Data);
            }
